...
import { mockTodos, TodoTask } from "@/lib/mockData";
import { Plus, Trash2, CheckCircle, Circle, Sparkles } from "lucide-react";

const TodoPage = () => {
  const [tasks, setTasks] = useState<TodoTask[]>(mockTodos);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-display font-bold text-glow-cyan">📝 Daily Quests</h1>
        <p className="text-muted-foreground text-sm mt-1">Complete your quests. Build your streak.</p>
      </div>

      {/* Overall Progress */}
      <div className="glass rounded-2xl p-6 glow-cyan text-center">
        <p className="text-sm text-muted-foreground mb-2">Today's Overall Progress</p>
        <div className="flex items-center justify-center gap-4">
          <div className="relative w-24 h-24">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="42" fill="none" stroke="hsl(230, 15%, 18%)" strokeWidth="8" />
              <circle
                cx="50"
                cy="50"
                r="42"
                fill="none"
                stroke={overallPercentage >= 80 ? "hsl(160, 100%, 45%)" : overallPercentage >= 50 ? "hsl(190, 100%, 50%)" : "hsl(25, 100%, 55%)"}
                strokeWidth="8"
                strokeLinecap="round"
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-muted-foreground">
              This % determines your streak color for today
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="glass rounded-xl p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={addTask}
            className="flex items-center justify-center gap-2 bg-primary text-primary-foreground px-5 py-2 rounded-xl font-display font-bold text-sm hover:opacity-90 transition-all self-end"
          >
      ...
      <div className="space-y-3">
        {tasks.map((task) => (
          <div
            key={task.id}
            className={`glass rounded-xl p-4 transition-all ${task.percentage === 100 ? "neon-border opacity-80" : ""}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  <p className={`font-medium text-sm ${task.percentage === 100 ? "line-through text-muted-foreground" : "text-foreground"}`}>
                    {task.title}
                  </p>
                  {task.description && (
            ...
            <div className="mt-3 flex items-center gap-3">
              <div className="flex-1 h-2 rounded-full bg-secondary overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-300 ${getProgressColor(task.percentage)}`}
