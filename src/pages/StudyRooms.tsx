...
const StudyRooms = () => {
  const [rooms, setRooms] = useState<Room[]>([
    { id: "1", name: "DSA Grinders 🔥", code: "DSA-420", isPrivate: false, members: ["PixelNinja", "CodeWiz", "ByteMe"], maxMembers: 10 },
    { id: "2", name: "System Design Gang", code: "SYS-069", isPrivate: true, members: ["PixelNinja", "ArchMaster"], maxMembers: 5 },
  ]);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-display font-bold text-glow-cyan">🏠 Study Rooms</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-all ${
              isPrivate ? "bg-neon-purple/20 text-neon-purple" : "bg-secondary text-muted-foreground"
            }`}
          >
          ...
          <button
            onClick={createRoom}
            className="flex items-center gap-2 bg-primary text-primary-foreground px-5 py-2 rounded-xl font-display font-bold text-sm hover:opacity-90 transition-all"
          >
          ...
          <div key={room.id} className="glass rounded-xl p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button className="bg-primary text-primary-foreground px-4 py-1.5 rounded-lg text-xs font-display font-bold hover:opacity-90 transition-all">
                  Join
                </button>
                <button
            ...
            <div className="flex gap-1 mt-3 flex-wrap">
              {room.members.map((m) => (
                <span key={m} className="bg-secondary text-foreground text-xs px-2 py-0.5 rounded-full">
                  {m}
                </span>
              ))}
            </div>
